...
import {
  collection, addDoc, onSnapshot, updateDoc, deleteDoc, doc, serverTimestamp
...
const bookedCell =
  "relative bg-red-200 text-red-900 after:content-[''] after:absolute after:inset-0 after:bg-[repeating-linear-gradient(45deg,transparent,transparent_6px,rgba(0,0,0,0.08)_6px,rgba(0,0,0,0.08)_12px)]";
...
function isBookedOn(day, roomId, bookings) {
  return bookings.some(
    (b) =>
      b.roomId === roomId &&
      isBefore(new Date(b.start), addDays(day, 1)) &&
      isAfter(new Date(b.end), day)
  );
}
...
function RoomMonth({ room, month, bookings, onTapFree, onTapBooked, isAdmin }) {
  ...
  return (
    <div className="mb-6">
      <div className="grid grid-cols-7 text-[11px] text-slate-500 px-1 pb-1">
        {["M", "T", "W", "T", "F", "S", "S"].map((d) => (
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((d, idx) => {
          ...
          const prev = addDays(d, -1),
            next = addDays(d, 1);
          const prevIn = isSameMonth(prev, monthStart),
            nextIn = isSameMonth(next, monthStart);
          ...
          const base =
            "h-7 flex items-center justify-center text-xs relative rounded-full";
          let cls = "";
          if (!inMonth) cls = "text-slate-300";
          else if (booked) {
            const left = !prevBooked,
              right = !nextBooked;
            cls = `${bookedCell} ${
              left ? "rounded-l-full pl-2" : ""
            } ${right ? "rounded-r-full pr-2" : ""}`;
          } else if (free) {
            const left = !prevFree,
              right = !nextFree;
            cls = `${freeCell} ${
...
export default function Availability({ isAdmin }) {
    ...
    const unsub = onSnapshot(
      ...
      (snap) => {
        ...
        snap.forEach((docSnap) =>
          cloud.push({ id: docSnap.id, ...docSnap.data() })
        );
        setBookings(cloud);
        try {
  ...
  async function handleRestore() {
    try {
      ...
      if (data && Array.isArray(data)) {
        ...
        alert("ℹ️ No valid backup found");
      }
    } catch (e) {
  ...
  function exportVisibleMonthCSV() {
    ...
    const rows = bookings
      .filter(
        (b) =>
          new Date(b.start) < monthEnd && new Date(b.end) > monthStart
      )
      .map((b) => {
        ...
        return {
          roomId: b.roomId,
          roomName: roomNameById[b.roomId] || "",
          guest: b.guest || "",
          note: b.note || "",
    ...
    a.href = url;
    a.download = `bookings_${format(month, "yyyy_MM")}.csv`;
  ...
  return (
    <div className="w-full max-w-md mx-auto p-3 pb-28">
      ...
      <div className="flex flex-wrap gap-2 justify-center mb-2">
        <button
          className="mt-1 px-3 py-1 text-xs rounded-lg border bg-white shadow-sm"
          onClick={exportVisibleMonthCSV}
        >
          Export Month
        </button>
        <button
