...
import { isSignedIntoDrive, signInDrive, signOutDrive } from './drive';

function getAdminList() {
  const multi = (process.env.REACT_APP_ADMIN_EMAILS || "").split(",").map(s => s.trim().toLowerCase()).filter(Boolean);
  const single = (process.env.REACT_APP_ADMIN_EMAIL || "").trim().toLowerCase();
  const set = new Set(multi);
  if (single) set.add(single);
  set.add("karmengrig@gmail.com"); // fallback so you always remain admin
