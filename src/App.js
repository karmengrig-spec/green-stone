...
import { isDriveConnected, signInDrive, signOutDrive } from './drive';

function getAdminList() {
  const multi = (process.env.REACT_APP_ADMIN_EMAILS || "")
    .split(",")
    .map(s => s.trim().toLowerCase())
    .filter(Boolean);
  const single = (process.env.REACT_APP_ADMIN_EMAIL || "").trim().toLowerCase();
  const set = new Set(multi);
  if (single) set.add(single);
  set.add("karmengrig@gmail.com"); // fallback: you always remain admin
...
export default function App(){
  ...
  const ADMIN_EMAILS = getAdminList();
          ...
          {driveReady && (
            driveSigned ? (
              <button
                ...
                onClick={async()=>{ await signOutDrive(); setDriveSigned(false); }}>
                Drive: Connected
              </button>
            ) : (
              <button
                className="px-2 py-1 text-xs rounded-lg border"
                onClick={async()=>{ await signInDrive(); setDriveSigned(true); }}>
                Connect Drive
              </button>
            )
          )}
          <button className="px-2 py-1 text-xs rounded-lg border" onClick={()=> signOut(auth)}>
            Sign out
          </button>
        </div>
      </div>
